...
Use the same retrieval tools as the Literature Agent (`osp-mcp.search_arxiv`, `search_semantic_scholar`, `search_google_scholar`, `search_acm`, `search_springer`, `search_ieee_xplore`, `search_sciencedirect`, native Web Search). You are encouraged to run targeted searches like:
