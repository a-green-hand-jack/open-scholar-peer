...
Each round must use **all available retrieval tools** (osp-mcp arxiv/semantic_scholar/google_scholar/dblp/pubmed/biorxiv/medrxiv/wos/scopus + native Web Search) with **different query formulations** per round. Queries used are listed in each round's `## Provenance`.
